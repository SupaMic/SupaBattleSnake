defmodule SupaBattleSnake do
  ...
  def convert_game_data(
        %{
          "game" => %{"id" => game_id, "timeout" => timeout_int} = _game_object,
          "turn" => turn_int,
          "board" =>
            %{
              "food" => food_list_of_maps,
              "height" => height_int,
              "snakes" => snakes_list_of_maps
            } = _board_object,
          "you" =>
            %{
              "body" => _you_list_of_coord_maps,
    ...
    game_converted = %GameBoard{
      game_id: game_id,
      timeout: timeout_int,
      board_height: height_int,
      turn: turn_int,
      board_food: food_list_of_maps,
      you_id: you_id,
      snakes: list_of_snakes,
      route_state: game_state,
      up: 0.5,
      right: 0.5,
      down: 0.5,
      left: 0.5
  ...
  def convert_to_list_of_snakes(snakes_list_of_maps, you_id) do
    ...
    |> Enum.map(fn map ->
      %Snake{
        ...
        you: GamePieces.is_you?(Map.get(map, "id"), you_id)
      }
    end)
  ...
  def determine_strategic_move(
        %GameBoard{up: up, right: right, down: down, left: left} = game_data
      ) do
    options = %{up: up, right: right, down: down, left: left}

    choose_boost_or_random =
      boosted_option_exists?(options)
    ...
    case choose_boost_or_random do
      ...
      false ->
        determine_random_move(game_data)
    end
  end
